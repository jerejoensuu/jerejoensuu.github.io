    ...
    const logoHtml = item.logo
      ? `
        <div class="logo-container">
          <img src="${item.logo}" alt="${item.logoAlt || item.company + " Logo"}" class="work-thumbnail">
        </div>`
      : "";
...
function computeCategorySummary(projects) {
  ...
  const otherBucket = all.find((c) => c.isOtherBucket);
  const normalBuckets = all.filter((c) => !c.isOtherBucket);
  ...
  if (cfg.collapseRemainderIntoOther) {
    ...
    if (otherBucket) otherCount += otherBucket.count;
  } else if (otherBucket) {
...
function showLoading() {
  ...
  if (grid) grid.classList.add("projects-grid-loading");
}
...
function hideLoading() {
  ...
  const grid = document.getElementById("projects-grid");
  if (grid) grid.classList.remove("projects-grid-loading");
}
...
function buildProjectCard(project) {
  ...
  projectCard.innerHTML = `
    ...
    <div class="tag-group-2">
      ${(project.topics || []).map((topic) => `<span class="tag3">${topic}</span>`).join("")}
    </div>
  `;
...
function rebuildProjectsGrid(projects) {
  const projectsGrid = document.getElementById("projects-grid");
  if (!projectsGrid) return;

  flagContainers.length = 0;
...
async function fetchProjects() {
  ...
  try {
    const response = await fetch("data/repos.json", { cache: "no-cache" });
    if (!response.ok)
    ...
    if (!hasAnyCards || oldFingerprint !== newFingerprint) {
      projectsGrid.setAttribute("data-fingerprint", newFingerprint);
      rebuildProjectsGrid(filteredProjects);
    } else {
      updateFlagAutoExpandOnScroll();
    }
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    ...
    if (!animationFrameId) updateTimer();
  }
});
